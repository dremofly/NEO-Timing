...
namespace Neo.SmartContract
{
    ...
    public sealed class NeoTimingContract : Framework.SmartContract
    {
       ...
       public static bool Register(string name, UInt160 owner)
        ...
        public static bool End(){
            ...
            if((user.EndTimeStamp - user.StartTimeStamp) >= (ulong)user.Interval){
                // Append(user.History,(int)user.StartTimeStamp);
                // Append(user.History,(int)user.EndTimeStamp);
                // Append(user.History,(int)user.Interval);
        ...
        public static void WithDraw(int exchange,string name) {
            ...
            StorageContext context = Storage.CurrentContext;
            StorageMap nameMap = new(context, Prefix_Name);
            if(nameMap.Get(tokenKey) is not null) {
